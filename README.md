...
The core methodology consists of: (i) Initializing probability values to (0.99,0.01) for malicious domains and (0.5,0.5) for unlabelled domains (this configuration prevents computational zeros that would occur with 0-valued initialization during multiplicative operations). (ii) Following the same computational procedure as the path-based inference approach to derive the domain relationship graph. (iii) Establishing inference relationships between adjacent nodes as specified in the following table:
